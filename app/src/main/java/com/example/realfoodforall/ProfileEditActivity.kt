...
import com.example.realfoodforall.databinding.ActivityProfileEditBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val pickImageFromGalleryForResult =
            registerForActivityResult(ActivityResultContracts.GetContent()) {
                binding.imageViewProfPicEdit.setImageURI(it)
                ...
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    if (image != null) {
                        Glide.with(applicationContext)
                            .load(image)
                            .into(binding.imageViewProfPicEdit)
        ...
        userRef.child(uid).child("username")
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    if (username != null) {
                        binding.editTextProfileName.setText(username)
                    } else {
        ...
        userRef.child(uid).child("email")
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    if (email != null) {
                        binding.editTextProfEmail.setText(email)
                    } else {
                        // Handle the case where the username is not found
        ...
        binding.buttonResetEdit.setOnClickListener(){

            Glide.with(applicationContext)
                .load(reset_img)
                .into(binding.imageViewProfPicEdit)

            binding.editTextProfileName.setText(reset_name)
            binding.editTextProfEmail.setText(reset_email)


        }

        binding.buttonChangeEmail.setOnClickListener(){
            val intent = Intent(this, ChangeEmailDonorPassword::class.java)
            startActivity(intent)
        }
        ...
        binding.buttonConfirmEdit.setOnClickListener {
            ...
            val newUsername = binding.editTextProfileName.text.toString()
            val newEmail = binding.editTextProfEmail.text.toString()
